...
function buildGrids() {
  ...
  const AUDIO_MAX_SCREEN = Math.max(screen.width || 0, screen.height || 0);

  const AUDIO_IS_IPAD =
    /iPad/i.test(AUDIO_UA) ||
    (AUDIO_HAS_TOUCH && /Macintosh/i.test(AUDIO_UA)); // iPadOS em modo desktop

  const AUDIO_IS_ANDROID_TABLET =
    /Android/i.test(AUDIO_UA) &&
    (!/Mobile/i.test(AUDIO_UA) || AUDIO_MIN_SCREEN >= 600);

  const AUDIO_IS_GENERIC_TABLET =
    /Tablet|Silk|Kindle|PlayBook|SM-T|Nexus 7|Nexus 10|Lenovo TB/i.test(AUDIO_UA);

  const AUDIO_IS_TABLET =
    AUDIO_IS_IPAD ||
    AUDIO_IS_ANDROID_TABLET ||
    AUDIO_IS_GENERIC_TABLET ||
    (AUDIO_HAS_TOUCH && AUDIO_MIN_SCREEN >= 700 && AUDIO_MAX_SCREEN >= 900);

  const AUDIO_IS_MOBILE_OR_TABLET =
    AUDIO_IS_TABLET ||
    /Android|iPhone|iPad|iPod|Mobile|Tablet/i.test(AUDIO_UA);
